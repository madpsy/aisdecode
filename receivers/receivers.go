...
type Settings struct {
    ...
    AlertsURL               string `json:"alerts_url"`              // URL for the alerts service
}
...
const (
	...
	ReceiverOfflineWeek = "RECEIVER_OFFLINE_WEEK" // Receiver has been offline for 7 days
)
...
func checkReceiverStatusChanges(prevPortLastSeenMap map[int]time.Time) {
	...
	now := time.Now()
	// Use the configured offline threshold or default to 24 hours
	offlineThresholdHours := settings.ReceiverOfflineHours
	if offlineThresholdHours <= 0 {
		offlineThresholdHours = 24 // Default to 24 hours if not specified
		log.Println("ReceiverOfflineHours not specified in settings.json, using default value of 24 hours")
	}
	offlineThreshold := time.Duration(offlineThresholdHours) * time.Hour
	
	// Define the 7-day threshold for the weekly offline alert
	weekOfflineThreshold := 7 * 24 * time.Hour
	...
	for rows.Next() {
		...
		if wasOnline && !isOnline {
			if err := logReceiverEvent(receiverID, ReceiverOffline); err != nil {
				...
				go sendOfflineAlert(receiverID, offlineThresholdHours)
			}
			...
			if err := logReceiverEvent(receiverID, ReceiverOnline); err != nil {
				...
				// Send alert to alerts service
				go sendOnlineAlert(receiverID)
			}
		} else if !isOnline { // If receiver is still offline, check if it's been offline for 7 days
			// Get the time when the receiver went offline
			offlineStartTime, err := getOfflineStartTime(receiverID)
			if err != nil {
				log.Printf("Error getting offline start time for receiver %d: %v", receiverID, err)
				continue
			}
			
			// If we have a valid offline start time and it's been more than 7 days
			if !offlineStartTime.IsZero() && now.Sub(offlineStartTime) >= weekOfflineThreshold {
				// Check if we've already sent a weekly offline alert recently
				lastWeeklyAlert, err := getLastWeeklyAlert(receiverID)
				if err != nil {
					log.Printf("Error getting last weekly alert for receiver %d: %v", receiverID, err)
					continue
				}
				
				// If we haven't sent a weekly alert yet, or it's been more than 7 days since the last one
				if lastWeeklyAlert.IsZero() || now.Sub(lastWeeklyAlert) >= weekOfflineThreshold {
					if err := logReceiverEvent(receiverID, ReceiverOfflineWeek); err != nil {
						log.Printf("Error logging OFFLINE_WEEK event for receiver %d: %v", receiverID, err)
					} else {
						log.Printf("Receiver %d has been OFFLINE for 7+ days", receiverID)
						
						// Send alert to alerts service
						go sendWeeklyOfflineAlert(receiverID)
					}
				}
			}
		}
...
// getOnlineStartTime returns the timestamp when the receiver came online after being offline
func getOnlineStartTime(receiverID int) (time.Time, error) {
	var startTime time.Time
				
	// Get the most recent RECEIVER_ONLINE event that follows a RECEIVER_OFFLINE event
	err := db.QueryRow(`
		SELECT e1.timestamp
		FROM receiver_events e1
		WHERE e1.receiver_id = $1 AND e1.event_type = 'RECEIVER_ONLINE'
		AND e1.id = (
			SELECT MIN(e2.id)
			FROM receiver_events e2
			WHERE e2.receiver_id = $1 AND e2.event_type = 'RECEIVER_ONLINE'
			AND e2.id > (
				SELECT MAX(e3.id)
				FROM receiver_events e3
				WHERE e3.receiver_id = $1 AND e3.event_type = 'RECEIVER_OFFLINE'
			)
		)
	`, receiverID).Scan(&startTime)
	
	if err == sql.ErrNoRows {
		// If there's no OFFLINE event, get the first ONLINE event
		err = db.QueryRow(`
			SELECT timestamp
			FROM receiver_events
			WHERE receiver_id = $1 AND event_type = 'RECEIVER_ONLINE'
			ORDER BY id ASC
			LIMIT 1
		`, receiverID).Scan(&startTime)
...
// getLastWeeklyAlert returns the timestamp of the last RECEIVER_OFFLINE_WEEK event
func getLastWeeklyAlert(receiverID int) (time.Time, error) {
	var alertTime time.Time
				
	err := db.QueryRow(`
		SELECT timestamp
		FROM receiver_events
		WHERE receiver_id = $1 AND event_type = 'RECEIVER_OFFLINE_WEEK'
		ORDER BY id DESC
		LIMIT 1
	`, receiverID).Scan(&alertTime)
	
	if err == sql.ErrNoRows {
		// No weekly alert events found
		return time.Time{}, nil
	}
				
	return alertTime, err
}

// sendOfflineAlert sends an alert to the alerts service when a receiver goes offline
func sendOfflineAlert(receiverID int, offlineHours int) {
	// Get the receiver details
	var rec Receiver
	err := db.QueryRow(`
		SELECT id, name, description, latitude, longitude, email, notifications, url
		FROM receivers
		WHERE id = $1
	`, receiverID).Scan(&rec.ID, &rec.Name, &rec.Description, &rec.Latitude, &rec.Longitude, &rec.Email, &rec.Notifications, &rec.URL)
				
	if err != nil {
		log.Printf("Error fetching receiver %d for offline alert: %v", receiverID, err)
		return
	}
	
	// Skip if notifications are disabled
	if !rec.Notifications {
		log.Printf("Skipping offline alert for receiver %d: notifications disabled", receiverID)
		return
	}
	...
	// Send the alert to the alerts service
	sendAlertToService(alertPayload)
}

// sendOnlineAlert sends an alert to the alerts service when a receiver comes back online
func sendOnlineAlert(receiverID int) {
	// Get the receiver details
	var rec Receiver
	err := db.QueryRow(`
		SELECT id, name, description, latitude, longitude, email, notifications, url
	...
	if err != nil {
		log.Printf("Error fetching receiver %d for online alert: %v", receiverID, err)
		return
	}
	
	// Skip if notifications are disabled
	if !rec.Notifications {
		log.Printf("Skipping online alert for receiver %d: notifications disabled", receiverID)
		return
	}
	...
	// Get the offline duration
	var offlineStart, onlineTime time.Time
	err = db.QueryRow(`
		SELECT
			(SELECT timestamp FROM receiver_events WHERE receiver_id = $1 AND event_type = 'RECEIVER_OFFLINE' ORDER BY id DESC LIMIT 1) as offline_start,
			(SELECT timestamp FROM receiver_events WHERE receiver_id = $1 AND event_type = 'RECEIVER_ONLINE' ORDER BY id DESC LIMIT 1) as online_time
	`, receiverID).Scan(&offlineStart, &onlineTime)
	
	var offlineHours float64
	if err == nil && !offlineStart.IsZero() && !onlineTime.IsZero() {
...
// sendWeeklyOfflineAlert sends an alert to the alerts service when a receiver has been offline for 7+ days
func sendWeeklyOfflineAlert(receiverID int) {
	// Get the receiver details
	var rec Receiver
	err := db.QueryRow(`
		SELECT id, name, description, latitude, longitude, email, notifications, url
	...
	if err != nil {
		log.Printf("Error fetching receiver %d for weekly offline alert: %v", receiverID, err)
		return
	}
	
	// Skip if notifications are disabled
	if !rec.Notifications {
		log.Printf("Skipping weekly offline alert for receiver %d: notifications disabled", receiverID)
		return
	}
...
// sendAlertToService sends an alert to the alerts service
func sendAlertToService(alertPayload map[string]interface{}) {
	// Check if alerts URL is configured
	if settings.AlertsURL == "" {
		log.Println("Cannot send alert: alerts_url not configured in settings")
		return
	}
				
	// Convert the payload to JSON
	jsonPayload, err := json.Marshal(alertPayload)
	if err != nil {
	...
	// Send the alert to the alerts service
	client := &http.Client{Timeout: 5 * time.Second}
	resp, err := client.Post(settings.AlertsURL+"/alert", "application/json", bytes.NewBuffer(jsonPayload))
	if err != nil {
		log.Printf("Error sending alert to alerts service: %v", err)
		return
	}
	defer resp.Body.Close()
	
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		log.Printf("Received non-2xx status from alerts service: %s", resp.Status)
		return
	}
	
	// Extract the receiver ID from the map for logging
	receiverMap, ok := alertPayload["receiver"].(map[string]interface{})
	if ok {
		if id, idOk := receiverMap["id"].(int); idOk {
			log.Printf("Successfully sent %s alert for receiver %d", alertPayload["alert_type"], id)
		} else {
			log.Printf("Successfully sent %s alert", alertPayload["alert_type"])
		}
	} else {
		log.Printf("Successfully sent %s alert", alertPayload["alert_type"])
	}
...
// Continue with the checkReceiverStatusChanges function
...
if !wasOnline && isOnline {
	if err := logReceiverEvent(receiverID, ReceiverOnline); err != nil {
		...
		} else if isOnline { // If receiver is still online, check if it's been online for 7 days
			// Get the first online event after the last offline event
			onlineStartTime, err := getOnlineStartTime(receiverID)
			if err != nil {
				log.Printf("Error getting online start time for receiver %d: %v", receiverID, err)
				continue
			}
			
			// If we have a valid online start time and it's been more than 7 days
			if !onlineStartTime.IsZero() && now.Sub(onlineStartTime) >= weekOnlineThreshold {
				// Check if we've already sent a weekly online alert recently
				lastWeeklyAlert, err := getLastWeeklyAlert(receiverID)
				if err != nil {
					log.Printf("Error getting last weekly alert for receiver %d: %v", receiverID, err)
					continue
				...
				// If we haven't sent a weekly alert yet, or it's been more than 7 days since the last one
				if lastWeeklyAlert.IsZero() || now.Sub(lastWeeklyAlert) >= weekOnlineThreshold {
					if err := logReceiverEvent(receiverID, ReceiverOnlineWeek); err != nil {
						log.Printf("Error logging ONLINE_WEEK event for receiver %d: %v", receiverID, err)
					} else {
						log.Printf("Receiver %d has been ONLINE for 7+ days", receiverID)
						
						// Send alert to alerts service
						go sendWeeklyOnlineAlert(receiverID)
					}
				}
			}
		}
