...
type FullMetrics struct {
	BytesReceivedBySource []struct {
		SourceIP string `json:"source_ip"`
		Count    int    `json:"count"`
	} `json:"bytes_received_by_source"`
	FailuresBySource []struct {
		SourceIP string `json:"source_ip"`
		Count    int    `json:"count"`
	} `json:"failures_by_source"`
	MessagesBySource []struct {
		SourceIP string `json:"source_ip"`
		Count    int    `json:"count"`
	} `json:"messages_by_source"`
	UniqueMMSIBySource []struct {
		SourceIP string `json:"source_ip"`
		Count    int    `json:"count"`
	} `json:"unique_mmsi_by_source"`
	...
	Top25DeduplicatedPerSource []struct {
		SourceIP string `json:"source_ip"`
		Count    int    `json:"count"`
	} `json:"top25_deduplicated_per_source"`
...
func main() {
	...
	// InfluxDB - attempt connection but don't fail if unavailable
	...
	if err != nil {
		log.Printf("Warning: Error creating InfluxDB client: %v - continuing without InfluxDB", err)
		influxClient = nil
	} else {
		...
		if err := ensureDatabase(settings.InfluxDB); err != nil {
			log.Printf("Warning: Could not create InfluxDB database %q: %v - continuing without InfluxDB", settings.InfluxDB, err)
			influxClient.Close()
			influxClient = nil
		}
	}
...
func queryAggregates(ip string) Aggregated {
	...
	for _, q := range queries {
		...
		if err != nil || res.Error() != nil || len(res.Results) == 0 || len(res.Results[0].Series) == 0 {
			continue
		}
...
func queryTimeSeriesGeneric(
	...
	if influxClient == nil {
		return nil, fmt.Errorf("InfluxDB client not available")
	}
